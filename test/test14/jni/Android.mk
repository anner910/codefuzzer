...
LOCAL_MODULE    := codefuzzer
# LOCAL_SRC_FILES变量必须包含将要编译打包进模块中的C或C++源代码文件。注意，你不用在这里列出头文件和包含文件，
# 因为编译系统将会自动为你找出依赖型的文件；仅仅列出直接传递给编译器的源代码文件就好。

LOCAL_CFLAGS :=  -O0 

MY_CPP_LIST := $(wildcard $(LOCAL_PATH)/../../../codefuzzer/core/*.c)
MY_CPP_LIST += $(wildcard $(LOCAL_PATH)/../../../codefuzzer/plus/*.c)
MY_CPP_LIST += $(wildcard $(LOCAL_PATH)/../../../codefuzzer/mutator/*.c)
...
#LOCAL_CPPFLAGS := -O0 ../../../codefuzzer/core/*.c 2323
